...
const tendermint33client_1 = require("./tendermint33client");
function defaultTestSuite(rpcFactory, expected) {
    ...
    it("gets the same tx hash from backend as calculated locally", async () => {
        ...
        const client = await tendermint33client_1.Tendermint33Client.create(rpcFactory());
        const tx = testutil_spec_1.buildKvTx(testutil_spec_1.randomString(), testutil_spec_1.randomString());
        const calculatedTxHash = adaptor_1.adaptor33.hashTx(tx);
        const response = await client.broadcastTxCommit({ tx: tx });
...
describe(`Tendermint33Client`, () => {
    ...
    it("can connect to a given url", async () => {
        ...
        // default connection
        {
            const client = await tendermint33client_1.Tendermint33Client.connect(url);
            const info = await client.abciInfo();
            expect(info).toBeTruthy();
            client.disconnect();
        }
        // http connection
        {
            const client = await tendermint33client_1.Tendermint33Client.connect("http://" + url);
            const info = await client.abciInfo();
            expect(info).toBeTruthy();
            client.disconnect();
        }
        // ws connection
        {
            const client = await tendermint33client_1.Tendermint33Client.connect("ws://" + url);
            const info = await client.abciInfo();
            expect(info).toBeTruthy();
            client.disconnect();
        }
...
//# sourceMappingURL=tendermint33client.spec.js.map